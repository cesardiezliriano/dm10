...
const AccordionIcon: React.FC = () => (
    <svg className="w-5 h-5 transition-transform duration-200 transform group-open:rotate-90" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
...
export const DataInputForm: React.FC<DataInputFormProps> = ({ onSubmit, isLoading, language, selectedBrandStyle, onBrandStyleChange }) => {
  ...
  const [campaignMarket, setCampaignMarket] = React.useState<string>('');
  const [additionalContext, setAdditionalContext] = React.useState<string>('');
  const [specificQuestions, setSpecificQuestions] = React.useState<string>('');
  
  const [uploadedCreatives, setUploadedCreatives] = React.useState<UploadedImage[]>([]);
  const [excelFileMessage, setExcelFileMessage] = React.useState<string | null>(null);
  const [creativeFileMessage, setCreativeFileMessage] = React.useState<string | null>(null);
  const [apiConnections, setApiConnections] = React.useState<ApiConnection[]>([]);
    ...
    reader.onload = (e) => {
      try {
        ...
        if (sheetData.length > 0) {
          const escapeMarkdownTableCharacters = (text: string) => {
            if (typeof text !== 'string') return text;
            return text.replace(/\|/g, '\\|').replace(/\n/g, ' '); // Also replace newlines to keep rows intact
          };

          const header = sheetData[0].map(cell => escapeMarkdownTableCharacters(String(cell))); 
          tableMarkdown += `| ${header.join(" | ")} |\n`;
          tableMarkdown += `| ${header.map(() => "---").join(" | ")} |\n`;
          for (let i = 1; i < sheetData.length; i++) {
            tableMarkdown += `| ${sheetData[i].map(cell => escapeMarkdownTableCharacters(String(cell))).join(" | ")} |\n`; 
          }
        } else {
  ...
  const stepHeaderStyle = "text-lg font-semibold text-[#0A263B] cursor-pointer list-none flex justify-between items-center w-full p-3";
  const stepContentStyle = "p-4 border-t border-gray-200";

  return (
    <form onSubmit={handleSubmit} className="space-y-2 text-[#0A263B]">
      
      {/* Step 1: Project Setup */}
      <details open className="bg-gray-50 border border-gray-200 rounded-lg group">
        <summary className={stepHeaderStyle}>
            <span className="flex items-center">
                <span className="flex items-center justify-center w-7 h-7 mr-3 text-sm font-bold text-white bg-[#F54963] rounded-full">1</span>
                <span style={labelStyle}>Configuración del Proyecto</span>
            </span>
            <AccordionIcon />
        </summary>
        <div className={stepContentStyle}>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" id="clientName" value={clientName} onChange={(e) => setClientName(e.target.value)} placeholder={CLIENT_NAME_PLACEHOLDER_FN(language)} className="w-full p-2 bg-white border border-[#ACB4B6] rounded-md shadow-sm focus:ring-[#F54963] focus:border-[#F54963] text-[#0A263B] placeholder-[#878E90] text-sm" disabled={isLoading} />
                </div>
                <div>
                    <label htmlFor="sector" className="block text-sm font-medium text-[#0A263B] mb-1" style={labelStyle}>
                    {getText(language, 'LABEL_SECTOR')}
                    </label>
                    <input type="text" id="sector" value={sector} onChange={(e) => setSector(e.target.value)} placeholder={SECTOR_PLACEHOLDER_FN(language)} className="w-full p-2 bg-white border border-[#ACB4B6] rounded-md shadow-sm focus:ring-[#F54963] focus:border-[#F54963] text-[#0A263B] placeholder-[#878E90] text-sm" disabled={isLoading} />
                </div>
                <div>
                    ...
                    <input type="text" id="campaignMarket" value={campaignMarket} onChange={(e) => setCampaignMarket(e.target.value)} placeholder={CAMPAIGN_MARKET_PLACEHOLDER_FN(language)} className="w-full p-2 bg-white border border-[#ACB4B6] rounded-md shadow-sm focus:ring-[#F54963] focus:border-[#F54963] text-[#0A263B] placeholder-[#878E90] text-sm" disabled={isLoading} />
                </div>
            </div>
             <div>
                ...
                <select id="brandStyle" value={selectedBrandStyle} onChange={(e) => onBrandStyleChange(e.target.value as BrandStyle)} disabled={isLoading} className="w-full p-3 bg-white border border-[#ACB4B6] rounded-md shadow-sm focus:ring-[#F54963] focus:border-[#F54963] text-[#0A263B] text-sm">
      ...
      <details className="bg-gray-50 border border-gray-200 rounded-lg group">
          <summary className={stepHeaderStyle}>
              <span className="flex items-center">
                <span className="flex items-center justify-center w-7 h-7 mr-3 text-sm font-bold text-white bg-[#F54963] rounded-full">2</span>
                <span style={labelStyle}>Añade tus Datos</span>
              </span>
              <AccordionIcon />
          </summary>
          <div className={stepContentStyle}>
            <ApiConnector language={language} isLoading={isLoading} connections={apiConnections} onConnect={handleApiConnect} onDisconnect={handleApiDisconnect} />
            <div className="mt-6">
                <h3 className="text-md font-medium text-[#0A263B] px-2 mb-4" style={labelStyle}>
                    O, añade datos manualmente
                </h3>
                    ...
                    <div key={block.id} className="p-4 border border-[#ACB4B6] rounded-md bg-white shadow-sm relative">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                          ...
                          <select id={`block-source-${block.id}`} value={block.source} onChange={(e) => handleDataBlockChange(block.id, 'source', e.target.value as DataSource)} disabled={isLoading} className="w-full p-2 bg-white border border-[#ACB4B6] rounded-md shadow-sm focus:ring-[#F54963] focus:border-[#F54963] text-[#0A263B] text-sm">
                      ...
                      <div>
                        ...
                        <textarea id={`block-kpis-${block.id}`} rows={5} value={block.kpis} onChange={(e) => handleDataBlockChange(block.id, 'kpis', e.target.value)} placeholder={PLACEHOLDER_DATA_BLOCK_KPIS(language)} disabled={isLoading} className="w-full p-3 bg-white border border-[#ACB4B6] rounded-md shadow-sm focus:ring-[#F54963] focus:border-[#F54963] text-[#0A263B] placeholder-[#878E90] text-sm custom-scrollbar"/>
                      </div>
                      <button type="button" onClick={() => handleRemoveDataBlock(block.id)} disabled={isLoading} title={getText(language, 'BUTTON_REMOVE_DATA_BLOCK')} className="absolute top-2 right-2 p-1 bg-gray-200 text-gray-600 rounded-full hover:bg-red-200 hover:text-red-800 transition-colors">
                ...
                <div className="flex items-center space-x-4 mt-4">
                  <button type="button" onClick={handleAddDataBlock} disabled={isLoading} className="flex items-center px-4 py-2 border border-dashed border-[#36A7B7] rounded-md text-sm font-medium text-[#36A7B7] bg-white hover:bg-[#36A7B7]/10 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-50 focus:ring-[#36A7B7] disabled:opacity-50">
                  ...
                  <div className="flex-grow">
                    ...
                    <input type="file" id="excelFile" ref={excelFileInputRef} accept=".xls,.xlsx,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={handleExcelFileChange} disabled={isLoading} className="block w-full text-sm text-[#6D7475] file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-[#F54963] file:text-white hover:file:bg-[#D93E52] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-50 focus:ring-[#F54963]"/>
                  </div>
                </div>
      ...
      {/* Step 3: Refine Analysis */}
       <details className="bg-gray-50 border border-gray-200 rounded-lg group">
            <summary className={stepHeaderStyle}>
                <span className="flex items-center">
                    <span className="flex items-center justify-center w-7 h-7 mr-3 text-sm font-bold text-white bg-[#F54963] rounded-full">3</span>
                    <span style={labelStyle}>Refina tu Análisis</span>
                </span>
                <AccordionIcon />
            </summary>
            <div className={`${stepContentStyle} space-y-6`}>
                ...
                <div>
                    ...
                    <input type="file" id="creativeFiles" ref={creativeFileInputRef} multiple accept="image/png, image/jpeg, image/gif" onChange={handleCreativeFileChange} disabled={isLoading || uploadedCreatives.length >= MAX_CREATIVES} className="block w-full text-sm text-[#6D7475] file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-[#36A7B7] file:text-white hover:file:bg-[#2A8E9A] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-50 focus:ring-[#36A7B7] disabled:opacity-50" aria-label={getText(language, 'LABEL_UPLOAD_CREATIVES_BUTTON')}/>
                    ...
                    <div className="mt-3">
                        ...
                        {uploadedCreatives.map(creative => (
                            ...
                            <button type="button" onClick={() => removeCreative(creative.name)} disabled={isLoading} className="absolute -top-2 -right-2 bg-[#F54963] text-white rounded-full h-5 w-5 flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity disabled:opacity-50" aria-label={`${getText(language, 'BUTTON_REMOVE_CREATIVE')} ${creative.name}`}>
                                X
                            </button>
                            </div>
                        ))}
                ...
                <div>
                    ...
                    <textarea id="additionalContext" rows={3} value={additionalContext} onChange={(e) => setAdditionalContext(e.target.value)} placeholder={ADDITIONAL_CONTEXT_PLACEHOLDER_FN(language)} className="w-full p-3 bg-white border border-[#ACB4B6] rounded-md shadow-sm focus:ring-[#F54963] focus:border-[#F54963] text-[#0A263B] placeholder-[#878E90] text-sm custom-scrollbar" disabled={isLoading} />
                </div>
                ...
                <div>
                    ...
                    <textarea id="specificQuestions" rows={3} value={specificQuestions} onChange={(e) => setSpecificQuestions(e.target.value)} placeholder={SPECIFIC_QUESTIONS_PLACEHOLDER_FN(language)} className="w-full p-3 bg-white border border-[#ACB4B6] rounded-md shadow-sm focus:ring-[#F54963] focus:border-[#F54963] text-[#0A263B] placeholder-[#878E90] text-sm custom-scrollbar" disabled={isLoading}/>
                </div>
            </div>
       </details>
